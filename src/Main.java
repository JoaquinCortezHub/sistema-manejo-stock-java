...
public class Main {
    ...
    private void mostrarProductos() {
        ...
        for (Producto producto : inventario.getProductos()) {
            System.out.printf("ID: %d, Nombre: %s, Precio: %.2f, Stock: %d%n",
                    producto.getNumArticulo(), producto.getNombre(), producto.getValor(), producto.getCantidad());
        }
        System.out.printf("Valor total del inventario: %.2f%n", inventario.getValorTotal()); //Formatear los valores de la lista
    }
    ...
    private void registrarVenta(Scanner scanner) {
        try {
            ...
            Producto productoVendido = inventario.getProductos().stream().filter(p -> p.getNumArticulo() == numArticulo)
                    .findFirst().orElseThrow(() -> new IllegalArgumentException("Producto no encontrado"));
    ...
    private void mostrarClientesYCompras() {
        ...
        for (Cliente cliente : clientes) {
            ...
            for (Venta venta : cliente.getCompras()) {
                System.out.printf("  - Producto: %s, Cantidad: %d, Total: %.2f%n",
                        venta.getProducto().getNombre(), venta.getCantidad(), venta.getPrecioTotal());
            }
        }
    }
